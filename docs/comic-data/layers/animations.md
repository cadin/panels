...
The horizontal position to animate the layer _to_. The animation will start _from_ the layer's [`x`](/docs/comic-data/layers#x) property (or `0` if not set).

### y
default: nil
{: .prop-default}

The vertical position to animate the layer _to_. The animation will start _from_ the layer's [`y`](/docs/comic-data/layers#y)  property (or `0` if not set).

### opacity
default: nil
{: .prop-default}

The opacity value to animate the layer _to_. The animation will start _from_ the layer's [`opacity`](/docs/comic-data/layers#opacity)  property (or `1` if not set).
